<template>
  <div class="wrapper">
    ...
    <div class="footer">
      <div>
        <!-- <van-icon size="25px" name="service-o" /> -->
      </div>
      <div class="inp">
        <van-cell-group>
          <van-field
...
export default {
  ...
  methods: {
    ...
    getParams() {
      ...
      if (this.$route.query.teacherName != null) {
  ...
  created() {},
  beforeCreate() {
    document.title = this.$route.query.teacherName;
  },
  updated() {
  ...
  mounted() {
    this.getParams();
    this.queryTeacherFeedDetails();
    this.$route.meta.title = this.teacherName;
  }
};
</script>
